...
mex -O 3rd_party/cs2/cs2mex.c %-O 3rd_party/cs2/cs2mex.c     %% compiling c implementation of push-relabel algorithm. It is downloaded from http://www.igsystems.com/cs2/index.html and then we mex'ed it to run faster in matlab.
...
load([datadir 'ETH-Bahnhof_ground_truth.mat']);
...
tic
dres_dp1       = tracking_dp_copy(dres, c_en, c_ex, c_ij, betta, thr_cost, max_it, 0);
dres_dp1.r     = -dres_dp1.id;
toc

tic
display('in DP tracking with nms in the loop...')
dres_dp_nms1   = tracking_dp_copy(dres, c_en, c_ex, c_ij, betta, thr_cost, max_it, 1);
dres_dp_nms1.r = -dres_dp_nms1.id;
toc

% tic
% display('in push relabel algorithm ...')
% dres_push_relabel   = tracking_push_relabel_copy(dres, c_en, c_ex, c_ij, betta, max_it);
...
dres_dp1           = sub(dres_dp1,            find(dres_dp1.fr           >1));
dres_dp_nms1       = sub(dres_dp_nms1,        find(dres_dp_nms1.fr       >1));
%%
%%% Evaluating
figure(1),
display('evaluating...')
...
[missr, fppi] = score(dres_dp1, gt, people);
semilogx(fppi,1-missr, 'y', 'LineWidth', 2);

[missr, fppi] = score(dres_dp_nms1, gt, people);
semilogx(fppi,1-missr, 'm');
...
legend('Dynamic programming', 'Dynamic programming with CNN','location', 'NorthWest')
...
% %%
fnum = max(dres.fr);
bboxes_tracked = dres2bboxes(dres_dp_nms1, fnum);  %% we are visualizing the "DP with NMS in the lop" results. Can be changed to show the results of DP or push relabel algorithm.
