function [res min_cs] = tracking_dp_copy(dres, c_en, c_ex, c_ij, betta, thr_cost, max_it, nms_in_loop)
...
%dres.c = betta - dres.r; %%%%%%%
Vdet = 0.2;
for i=1:length(dres.r)
    if dres.r(i)<Vdet
        dres.r(i) = -dres.r(i)/Vdet+1;
    else
        dres.r(i) = (1-dres.r(i))/(1-Vdet)-1;
    end
end 
dres.c = dres.r;
...
while (min_c < thr_cost) && (it < max_it)
  ...
  for ii=1:length(redo_nodes)
    ...
    [min_cost j] = min((dres.srf(i).inds)' + dres.c(i) + dres.dp_c(f2));
    min_link = f2(j);
